...
import fr.ecp.is1220.myvelib.part1.NoCard;
import fr.ecp.is1220.myvelib.part1.User;

class NoCardCostTest {
	...
	void whenComputingCostThenNoExceptionThrown() {
		...
		alice.setRegistrationCard(new NoCard(alice));
	...
	@Test
	void whenUnknownBicycleTypeThenExceptionThrown() {
		User alice = new User("Alice");
		alice.setCreditCard(new CreditCard(alice));
		alice.setRegistrationCard(new NoCard(alice));
		Date rentTime = new Date();
		...
		try {
			alice.getCost().cost("banana", rentTime, returnTime, alice.getRegistrationCard());
			throw new Exception("Test fail");
		}
		catch (Exception e) {
			assertTrue(e.getMessage().contains("Unknown bicycle type"));
		}
	}
